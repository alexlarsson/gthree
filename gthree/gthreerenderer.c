...
static void
before_render_skybox (GthreeObject                *object,
                      GthreeScene                 *scene,
                      GthreeCamera                *camera)
{
  g_print ("before_render_skybox\n");
  const graphene_matrix_t *camera_world_matrix;
  graphene_matrix_t m;
  graphene_point3d_t camera_offset;

  g_print ("skybox world matrix:\n");
  graphene_matrix_print (gthree_object_get_world_matrix (GTHREE_OBJECT (object)));
  ...
  if (bg_texture && GTHREE_IS_CUBE_TEXTURE (bg_texture))
    {
      ...
      if (priv->bg_box_mesh == NULL)
        {
          ...
          gthree_object_set_before_render_callback (GTHREE_OBJECT (box_mesh), before_render_skybox);

          /* TODO: Unrealize, and then re-realize as neeed */
      ...
      /* TODO: Only re-set texture when it changes */
      shader = gthree_material_get_shader (gthree_mesh_get_material (priv->bg_box_mesh));
      ...
      object_buffers = gthree_object_get_object_buffers (GTHREE_OBJECT (priv->bg_box_mesh));
      g_print ("skybox buffers: %p\n", object_buffers);
      for (l = object_buffers; l != NULL; l = l->next)
        {
          GthreeObjectBuffer *buffer_obj = l->data;
          GthreeMaterial *material = gthree_object_buffer_resolve_material (buffer_obj);
